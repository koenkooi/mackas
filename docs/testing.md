...
`require_root`, `set_get_unset`, `setup_e2e`,
...
prune/lock/dump (via the shared `kas_shell_ro()` helper), and a hand-typed
`kas-container` all auto-start it before handing off, the same check
