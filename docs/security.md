...
HTTP mirror server); and the `tools/` scripts.
...
- **No default secrets or open ports.** The mirror server is off by default;
  when enabled it warns loudly unless the operator configures a credential or
  an IP allowlist (allowlist-only is a supported, stated trade on a trusted
  LAN).
...
## The optional mirror server

`mackas-mirrord` is the one network-facing component and is hardened
accordingly: it serves read-only, confines all file access beneath its
configured roots (no path traversal), supports TLS, uses a PBKDF2-derived
credential with constant-time comparison, and bounds request headers and bodies.
It is still **optional and off by default** — you opt in.
