...
| `tests/retrieve.bats` | `mackas retrieve` against a fake `container` that models the existence probe and the copy: the `-u 0:0` copy recipe, `--dest` defaulting/override, the "no build has run yet" message, `--dry-run` mutating nothing, the `buildstats`/`logs`/`deploy` objects (repeatable, at least one required), the one-VM refusal when a running container holds the volume, and the `buildstats analyze` hint. |
| `tests/buildstats_analyze.bats` | `mackas buildstats analyze` (no `container` involved): resolving the default `$MACKAS_BASE/artifacts/buildstats` path vs an explicit PATH, and the non-fatal warning when the analyzer script is missing. |
| `tests/test_buildstats_analyze.py` | `tools/mackas-buildstats-analyze`: that a task's CPU includes its **child** rusage (or every compile looks free), `maxrss` taking the larger of own/child, and a truncated task file not sinking the report. `unittest`, stdlib. |
