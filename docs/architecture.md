...
- **replays the project it was generated for** into that live recompute, but only bakes that pin in when the selection was explicit (`--project`/`$MACKAS_PROJECT_SELECT`) **and** exactly one project claims this `MACKAS_ROOT` — counting both pinned projects and one configured through the default search path ([#114](https://github.com/koenkooi/mackas/issues/114)). There is one wrapper per root, not one per project, so a pin baked under a shared root would hand every sibling's hand-typed build the wrong project's volumes; with more than one claimant the pin stays empty and each call re-derives;
